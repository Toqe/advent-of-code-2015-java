...
class Day06Part2 {
    ...
    public fun apply(instruction : Triple<ChangeLightType, Pair<Int, Int>, Pair<Int, Int>>, lights : Array<Array<Int>>) {
        for (y in instruction.second.second..instruction.third.second) {
            for (x in instruction.second.first..instruction.third.first) {
                when (instruction.first) {
                    ChangeLightType.TURN_ON -> lights[y][x] += 1
                    ChangeLightType.TURN_OFF -> lights[y][x] = kotlin.math.max(0, lights[y][x] - 1)
                    ChangeLightType.TOGGLE -> lights[y][x] += 2
                }
            }
        }
    }
